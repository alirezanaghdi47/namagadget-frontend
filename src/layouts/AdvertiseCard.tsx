      ...
      >
        <Link href="/account/my-advertises/advertise" className="flex h-full w-full items-center justify-start gap-2">
          <div className="relative min-w-32 h-32 py-2">
            ...
            {status && (
                <div className={`absolute bottom-4 left-1/2 -translate-x-1/2 flex items-center justify-center p-2 bg-${status.type} w-28 rounded-lg`}>
                  <span className={`text-nowrap text-xs font-bold text-${status.type}-foreground`}>منتشر شده</span>
                </div>
            )}
          </div>
